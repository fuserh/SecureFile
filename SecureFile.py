    ...
    def wipe_sensitive_data(*args) -> None:
        ...
        for arg in args:
            if isinstance(arg, bytearray):
                ...
                # 对于 bytes，创建一个 bytearray 副本进行擦除
                try:
                    temp_buffer = bytearray(arg)
                    SecureMemory.secure_erase(temp_buffer)
                    # 注意：原始 bytes 对象本身是不可变的，无法直接擦除其内存
                except:
                    pass
    ...
    def check_and_wait(self):
        ...
        with self.lock:
            if self.attempts >= self.max_attempts:
                delay = self.base_delay * (2 ** (self.attempts - self.max_attempts))
...
class SecureFileEncryptor:
    ...
    def __init__(self, password: Optional[str] = None):
        ...
        # 密码尝试限制器
        self.limiter = PasswordAttemptLimiter()
        # 如果提供了密码，派生主密钥
        if password:
            self._derive_master_keys(password)

    def __del__(self):
        """析构函数，确保敏感数据被清除"""
        self._clear_master_keys()

    def _derive_master_keys(self, password: str) -> None:
        """从密码安全派生主密钥和HMAC密钥"""
        # 生成随机盐
    ...
    def create_real_hevc_video(self, encrypted_data: bytes, output_path: str) -> bool:
        ...
        """
        # 使用 delete=True 并手动管理临时文件路径
        temp_bin_path = None
        temp_yuv_path = None
        try:
            # 创建临时二进制文件
            with tempfile.NamedTemporaryFile(suffix='.bin', delete=True) as temp_bin:
                temp_bin_path = temp_bin.name
                temp_bin.write(encrypted_data)
                temp_bin.flush() # 确保数据写入磁盘
                ...
                if data_size < bytes_per_frame * frame_count:
                    ...
                    data_size = len(encrypted_data)
                # 创建临时原始视频文件
                with tempfile.NamedTemporaryFile(suffix='.yuv', delete=True) as temp_yuv:
                    temp_yuv_path = temp_yuv.name
                    temp_yuv.write(encrypted_data[:bytes_per_frame * frame_count])
                    temp_yuv.flush() # 确保数据写入磁盘
            ...
            # 尝试删除临时文件（即使 NamedTemporaryFile 设置了 delete=True，手动删除更可靠）
            # 注意：在 Windows 上，如果文件正在被 FFmpeg 使用，可能无法立即删除。
            # 通常 NamedTemporaryFile 会在退出 with 块时自动删除，但为了确保，这里也尝试。
            # 如果 FFmpeg 进程仍在运行，删除可能会失败，但这在同步调用下不太可能。
            pass # NamedTemporaryFile 应该已经处理了
    ...
    def process_for_encryption(self, folder_path: str, output_dir: str) -> Dict[str, Any]:
        """处理整个加密流程（安全优先）"""
        temp_dir = None
        original_path_for_deletion = folder_path # 记录原始路径，用于后续安全删除
        ...
        try:
            ...
            if is_folder:
                ...
                file_to_encrypt = folder_path # 如果输入是文件，则直接加密

            # 3. 分割文件
            chunks = self.split_file_into_random_chunks(file_to_encrypt)
            ...
            for i, chunk in enumerate(chunks):
                ...
                self.encryption_info["original_filename"] = os.path.basename(folder_path.rstrip(os.sep)) if is_folder else os.path.basename(folder_path)
                # 生成随机文件名
            ...
            # 7. 安全删除原始文件/文件夹
            if is_folder:
                 self.secure_delete_file(original_path_for_deletion, passes=3)
            else:
                 self.secure_delete_file(original_path_for_deletion, passes=3)
    ...
    def extract_video_data(self, video_path: str) -> bytes:
        ...
        try:
            ...
            # NamedTemporaryFile 应该已经处理了，但为了确保
            pass
    ...
    def process_for_decryption(self, video_files: List[str], metadata_file: str, output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.limiter.record_attempt() # 记录失败尝试
                return {
                    "success": False,
            ...
            for i, (encrypted_data, chunk_info) in enumerate(sorted_chunks):
                ...
                if not self._verify_chunk_hmac(
                    ...
                    failed_message = f"块 {i} 的完整性验证失败 - 数据可能被篡改"
                    break # 立即停止解密
                ...
                except ValueError as ve: # 捕获特定的解密错误
                    decryption_failed = True
                    failed_message = f"解密失败 (块 {i}): {str(ve)}"
                    self.limiter.record_attempt() # 记录失败尝试
                    break # 立即停止解密
                finally:
                    # 立即清除敏感数据 [[26]]
            ...
            if decryption_failed:
                 ...
                 for chunk in decrypted_chunks:
                     SecureMemory.wipe_sensitive_data(bytearray(chunk))
                 return {
                     "success": False,
            ...
            # import traceback
            # print(f"Decryption error: {str(e)}\n{traceback.format_exc()}") # 移除详细错误信息打印
            return {
                "success": False,
...
class ScrollableFrame(ttk.Frame):
    ...
    def __init__(self, container, *args, **kwargs):
        ...
        self.scrollable_frame.bind("<Configure>", self._configure_frame)
...
class SecureEncryptorApp:
    def __init__(self, root):
        ...
        # 设置窗口可调整大小
        self.root.minsize(700, 500)  # 设置最小窗口尺寸
        ...
        # 标题（保持不变）
        ttk.Label(self.main_frame, text="SecureFile", style='Header.TLabel').pack(pady=(0, 5), fill="x")
        ttk.Label(self.main_frame, text="企业级安全文件加密与伪装工具", style='SubHeader.TLabel').pack(pady=(0, 15), fill="x")
        ...
        # 状态栏（保持不变，但需要放在根窗口而不是main_frame中）
    ...
    def setup_encrypt_tab(self) -> None:
        """设置加密选项卡的UI（调整为适应滚动）"""
        # 文件选择区域
        file_frame = ttk.LabelFrame(self.encrypt_tab, text="选择要加密的文件或文件夹")
        file_frame.pack(fill=tk.X, padx=15, pady=10, expand=True)
        ...
        password_frame.pack(fill=tk.X, padx=15, pady=10, expand=True)
        ...
        self.password_strength_label = ttk.Label(password_frame, textvariable=self.password_strength_var, 
                 foreground="#d93025", font=('Segoe UI', 9))
        self.password_strength_label.pack(anchor=tk.W, padx=10, pady=(0, 5))
        self.password_entry.bind('<KeyRelease>', self.check_password_strength)
        
        # 输出区域
        output_frame = ttk.LabelFrame(self.encrypt_tab, text="输出设置")
        output_frame.pack(fill=tk.X, padx=15, pady=10, expand=True)
        ...
        info_frame.pack(fill=tk.X, padx=15, pady=10, expand=True)
        info_text = (
            "• 本工具使用企业级安全措施保护您的数据\n"
            "• 每个数据块使用唯一密钥（从主密钥安全派生）\n"
            "• 使用AEAD加密模式（AES-GCM和ChaCha20-Poly1305）提供认证加密\n"
        ...
        button_frame.pack(fill=tk.X, padx=15, pady=15, expand=True)
    ...
    def setup_decrypt_tab(self) -> None:
        ...
        metadata_frame.pack(fill=tk.X, padx=15, pady=10, expand=True)
        ...
        self.decrypt_log_text = scrolledtext.ScrolledText(log_text_frame, wrap=tk.WORD,
                                                        font=('Consolas', 9), bg="#ffffff", fg="#333335")
        self.decrypt_log_text.pack(fill=tk.BOTH, expand=True)
    ...
    def browse_folder(self) -> None:
        ...
        if not folder_selected:
             ...
             if file_selected:
                 ...
                 self.log(f"已选择文件: {file_selected}")
        else:
    ...
    def check_password_strength(self, event=None) -> None:
        ...
        if hasattr(self, 'password_strength_label'):
             self.password_strength_label.configure(foreground=color)
        else:
            # 创建标签（如果不存在）
            ...
            self.password_strength_label.pack(anchor=tk.W, padx=10, pady=(0, 5)) # 添加到密码框架中

    def start_encryption(self) -> None:
        """开始加密过程（在新线程中运行）"""
        # 验证输入
        if not self.folder_path.get():
            messagebox.showerror("错误", "请选择要加密的文件或文件夹")
            return
    ...
    def run_decryption(self, password: Optional[str]) -> None:
        ...
        try:
            ...
            # 清除GUI中的密码
            self.root.after(0, lambda: self.decrypt_password_var.set(""))
            # 重新启用按钮
